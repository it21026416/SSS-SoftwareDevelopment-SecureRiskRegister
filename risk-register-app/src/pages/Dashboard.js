...
function Dashboard() {
	...
	const calculateAverage = (riskData, stat, setUpdate) => {
		const lowQty = riskData.filter((risk) => risk[stat] === "low").length; const mediumQty = riskData.filter((risk) => risk[stat] === "medium").length;
	...
	// If the user is already signed in, clicking will log him out, and vice-versa
	const handleSignInClick = () => {
		if (localStorage.getItem("signedIn")) {
			// Remove is all user data from local storage
			...
			// Have to do this so that there is a re-render
		...
		<main className={`${styles.dashboard} ${darkMode}`}>
			...
			{HideMenu && (
				<div className={styles.menu}>
					...
					<Register funcToRun={setRegisterView} />

					<button className={styles.menuBtn} onClick={handleSignInClick}>
			...
			{/* Left Menu Buttons*/}
			{register && <RegisterView registerView={setRegisterView} />}
			{signedIn && <SignInView signedInView={signedInView} />}
